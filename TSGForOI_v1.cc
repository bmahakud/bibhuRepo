...
void TSGForOI::produce(edm::StreamID sid, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  for (unsigned int l2TrackColIndex(0);l2TrackColIndex!=l2TrackCol->size();++l2TrackColIndex){
   ...
   unsigned int numHitbasedSeedsMade=0;
	...
	findSeedsOnLayer("TEBip",NumCompatibleHitsTECtsIP,NumCompatibleHitsTEBtsIP,NumCompatibleHitsTECtsMuS,NumCompatibleHitsTEBtsMuS,numHitlessSeedsMade,numHitbasedSeedsMade,tTopo, **it, tsosAtIP,  *(propagatorAlong.get()), *(propagatorOpposite.get()), l2, 
    ...
    //cout<<"num TOB Layers"<<numTOBLayers<<endl;
      if (useBoth){
    ...
    //  layerCount = 0;
	      ...
	      for (auto it=tob.rbegin(); it!=tob.rend(); ++it) {	//This goes from outermost to innermost layer
			LogTrace("TSGForOI") << "TSGForOI::produce: looping in TOB layer " << layerCount << endl; 
      int numseed=out->size();
			findSeedsOnLayer("TEBmus",NumCompatibleHitsTECtsIP,NumCompatibleHitsTEBtsIP,NumCompatibleHitsTECtsMuS,NumCompatibleHitsTEBtsMuS,numHitlessSeedsMade,numHitbasedSeedsMade,tTopo, **it, tsosAtMuonSystem,  *(propagatorOpposite.get()), *(propagatorOpposite.get()), l2, 
			 estimatorH, measurementTrackerH, numSeedsMade, numOfMaxSeeds, layerCount, analysedL2, out);
	...
	LogTrace("TSGForOI") << "TSGForOI::produce: looping in TEC+ layer " << layerCount << endl; 
        nTECLayers++;
        int numseed=out->size();
	findSeedsOnLayer("TECip",NumCompatibleHitsTECtsIP,NumCompatibleHitsTEBtsIP,NumCompatibleHitsTECtsMuS,NumCompatibleHitsTEBtsMuS,numHitlessSeedsMade,numHitbasedSeedsMade,tTopo, **it, tsosAtIP, *(propagatorAlong.get()), *(propagatorOpposite.get()), l2, 
			 estimatorH, measurementTrackerH, numSeedsMade, numOfMaxSeeds, layerCount, analysedL2, out);
    ...
    //layerCount = 0;
		numSeedsMade=0;
			...
			LogTrace("TSGForOI") << "TSGForOI::produce: looping in TEC+ layer " << layerCount << endl; 
                  ...
                  int numseed=out->size();
			findSeedsOnLayer("TECmus",NumCompatibleHitsTECtsIP,NumCompatibleHitsTEBtsIP,NumCompatibleHitsTECtsMuS,NumCompatibleHitsTEBtsMuS,numHitlessSeedsMade,numHitbasedSeedsMade,tTopo, **it, tsosAtMuonSystem,  *(propagatorOpposite.get()), *(propagatorOpposite.get()), l2, 
			 estimatorH, measurementTrackerH, numSeedsMade, numOfMaxSeeds, layerCount, analysedL2, out);
   ...
   //layerCount = 0;
		numSeedsMade=0;
			...
			LogTrace("TSGForOI") << "TSGForOI::produce: looping in TEC- layer " << layerCount << endl; 
             ...
             int numseed=out->size();
			findSeedsOnLayer("TECmus",NumCompatibleHitsTECtsIP,NumCompatibleHitsTEBtsIP,NumCompatibleHitsTECtsMuS,NumCompatibleHitsTEBtsMuS,numHitlessSeedsMade,numHitbasedSeedsMade,tTopo, **it, tsosAtMuonSystem,  *(propagatorOpposite.get()), *(propagatorOpposite.get()), l2, estimatorH, measurementTrackerH, numSeedsMade, numOfMaxSeeds, layerCount, analysedL2, out);
...
void TSGForOI::findSeedsOnLayer(std::string SubDet,
  ...
  if (makeSeedsFromHits(SubDet,NumCompatibleHitsTECtsIP,NumCompatibleHitsTEBtsIP,NumCompatibleHitsTECtsMuS,NumCompatibleHitsTEBtsMuS,numHitbasedSeedsMade,tTopo, layer, tsosAtIP, *out, propagatorAlong, *measurementTrackerH, estimatorH, numSeedsMade, errorSFHits, l2->eta()))  ++layerCount; 
}
...
int TSGForOI::makeSeedsFromHits(std::string SubDet,
                                unsigned int& NumCompatibleHitsTECtsIP,
                                unsigned int& NumCompatibleHitsTEBtsIP,
                                unsigned int& NumCompatibleHitsTECtsMuS,
                                unsigned int& NumCompatibleHitsTEBtsMuS,
  ...
  //		Error Rescaling:
 //cout<<"starting findhitbased seed ................................."<<endl;
  TrajectoryStateOnSurface onLayer(tsosAtIP);
 ...
 // } 
  ...
  //cout<<"meas size before creating the size:   "<<meas.size()<<endl;
